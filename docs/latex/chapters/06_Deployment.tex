...
\subsection{Sicurezza e Configurazione}
L'inoltro delle richieste dal backend NestJS all'API di Hugging Face è protetto e configurato mediante variabili d'ambiente:
\begin{itemize}
    \item \texttt{AI\_API\_URL}: Indica l'URL pubblico dello Space di Hugging Face.
    \item \texttt{HF\_TOKEN}: Un token autorizzativo (\textit{Bearer Token}) passato negli header delle richieste per validarne la provenienza, prevenendo utilizzi non autorizzati ai modelli.
\end{itemize}
...
\begin{itemize}
    \item Inizializza i log verificando la correttezza delle configurazioni ambientali per \texttt{AI\_API\_URL} e \texttt{HF\_TOKEN}.
    \item Espone il metodo principale \texttt{predict(endpoint, payload)}, che compone la chiamata REST inserendo l'autorizzazione di sicurezza e gestisce efficacemente le risposte o gli errori causati da anomalie di rete (come richieste rifiutate o codice ritornato nei \textit{4xx/5xx}).
