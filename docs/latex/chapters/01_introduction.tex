...
% ──────────────────────────────────────────────────────────────────────────────
\clearpage
\section{Integrazione con Modelli AI: Funzionalità Previste}
\label{sec:funzionalita_implementate}
Il progetto SINTON-IA prevede l'integrazione di tre modelli di Intelligenza Artificiale, ciascuno progettato per colmare una specifica lacuna operativa dell'attuale piattaforma SINTONIA. In questa fase introduttiva ci si limita a descrivere la finalità e il tipo di task affrontato da ciascun modello, rimandando ai capitoli successivi l'analisi tecnica e architetturale.
...
\label{subsec:churn}
Il terzo e ultimo modello adotta un \textbf{approccio di tipo genetico-evolutivo} per affrontare il problema della previsione dell'abbandono dell'applicazione (\textit{churn}) da parte dei pazienti durante la fase di attesa.

L'obiettivo è stimare la probabilità che un utente smetta di utilizzare attivamente SINTONIA prima della presa in carico, evento che comprometterebbe la raccolta dei dati necessari al corretto funzionamento del triage dinamico. Sulla base delle predizioni del modello, il sistema invierà \textit{notifiche strategiche e personalizzate} agli utenti a rischio di abbandono, incentivandoli a mantenere un utilizzo costante della piattaforma.

Questo modello riveste un ruolo trasversale: garantendo la continuità nell'acquisizione dei dati comportamentali, esso contribuisce indirettamente all'efficacia di tutti gli altri modelli e al funzionamento complessivo del sistema di prioritizzazione clinica.
